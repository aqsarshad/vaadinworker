 ...
 * view, giving a more responsive feeling for the client.  
 *
...
public class FibonacciComputer extends VaadinWorker implements ProgressListener {
	...
	// Additionally save the current results in order to keep the 
	// ProgressListener free from any "heavy" logic
	private int counter;
	...
	public void runInBackground() {
		...
		for(counter = 3; counter <= numberCount; counter++) {
	...
	public void workProgressed(int progress, String state, VaadinWorker worker) {
		...
		// Now just add the newly computed numbers
		Object itemId = container.addItem();
		container.getItem(itemId).getItemProperty(COUNTER).setValue(counter);
		container.getItem(itemId).getItemProperty(DATA).setValue(current);
	}
